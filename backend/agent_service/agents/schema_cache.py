...
def invalidate(connection_id: str) -> None:
    """
    Evict all cached data for this connection from in-process cache, Redis, AND
    the filesystem fallback cache. Call after a schema re-crawl or when a
    migration is detected.

    The filesystem tier has no TTL check on read (see _fs_read) — without
    clearing it here too, a stale pre-enrichment EnrichedSchema written before
    this crawl's metadata_extractor pass finished can be served indefinitely
    after this call, and would even re-warm Redis with the stale blob on the
    next cache miss.
    """
    ...
    try:
        ...
        for p in _glob.glob(str(_fs_cache_dir() / f"schema_{connection_id}_*.json")):
            try:
                ...
                print(f"[schema_cache] invalidated filesystem cache file={p}", flush=True)
            except Exception as _fe:
                print(f"[schema_cache] ⚠ failed to remove filesystem cache file={p}: {_fe}", flush=True)
    except Exception as _ge:
        print(f"[schema_cache] ⚠ filesystem cache invalidation glob failed (non-fatal): {_ge}", flush=True)
    ...
    for t in tables:
        ...
        for c in all_cols:
            ...
            enriched_cols.append({
                ...
                # is_kpi_metric/is_dimension were generated by metadata_extractor and
                # persisted, but never copied through to compact_tables before now —
                # graph_rag_retriever.py had nothing to read (see gap-analysis Phase 1c).
                "is_kpi_metric": db_col.is_kpi_metric if db_col else None,
                "is_dimension": db_col.is_dimension if db_col else None,
...
- use_for: short phrases describing what kinds of questions this table correctly answers
- never_use_for: short phrases describing what this table must NOT be used to answer
  (e.g. a table with no financial columns should say "billing amounts", "revenue totals")
- is_fact_table: true if this is a fact/transaction/event table (many rows, numeric
  measures, joined to dimension tables); false for a lookup/dimension/reference table
- business_name: a short human-friendly name for this table (e.g. "Job Applications")
...
def _build_concept_index_heuristic(
    ...
    for tbl in compact_tables:
        ...
        # business_name/purpose live on table_semantics (sem), not on the compact_tables
        # entry itself — compact_tables never carries a table-level "business_name" or
        # "description" key (only sem["purpose"] does). Reading from `tbl` here always
        # returned "", silently dropping this signal — see gap-analysis Phase 0a.
        tbl_biz: str = sem.get("business_name", "") or ""
        tbl_domain_tokens: list[str] = []
        for raw in (tbl_desc, tbl_biz, sem.get("purpose", "") or ""):
